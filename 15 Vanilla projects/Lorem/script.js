...
let randomPara = document.querySelector('.randomPara');

let myInput = document.getElementById('numberOfParas');
...
myBtn.addEventListener('click', () => {
    ...
    if (myInput.value == 1) {
        randomPara.innerText = '' + myParas[Math.floor(Math.random() * myParas.length)];
    } else if (myInput.value > 1 && myInput.value < 10) {
        for (let i = 0; i < myInput.value; i++) {
            randomPara.innerText += myParas[i];
            randomPara.innerHTML += `<br>`;
            randomPara.innerHTML += `<br>`;
        }
    }
})